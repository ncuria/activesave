 ...
 *  Version 0.1
 *
 *  Copyright © 2015 Nick Curia; released under the MIT license
 *
 **/
(function () {
    var window = this,
    ...
    $.fn.activesave = function (options) {
        var settings = {
            scope: 'global',
        ...
        return this.filter('form[id]').each(function () {
            // Register form
            var registeredForm = {};
            asave.registeredForms[$(this).prop('id')] = registeredForm;
            ...
            $(this).off('submit').on('submit', function () {
                var registeredForm = asave.registeredForms[$(this).prop('id')];
    ...
    asave.getKey = function (subscope, scope) {
        scope = scope || 'global';
    ...
    asave.retrieve = function (name, subscope, scope) {
        var key = asave.getKey(subscope, scope);
    ...
    asave.remove = function (name, subscope, scope) {
        var key = asave.getKey(subscope, scope);
    ...
    asave.set = function (name, value, subscope, scope) {
        scope = scope || 'global';

        // First attempt to set on the form itself
        var formSet = false;
        if (asave.registeredForms) {
            for (var formName in asave.registeredForms) {
                var registeredForm = asave.registeredForms[formName];
                if (registeredForm.scope === scope && registeredForm.subscope === subscope || null) {
                    asave.setField(registeredForm, registeredForm.form, name, value);
                    formSet = true;
                    break;
                }
            }
    ...
    asave.deferredValues = asave.deferredValues || {};

    // Set a named value in the form
    asave.setField = function (target, form, name, value) {
        var $form = $(form);
                ...
                if ($thisElem.prop('type') == 'checkbox') {
                    ...
                    if ($thisElem.val() !== value) {
                        ...
                        if ($thisElem.data('async-load') === true || $thisElem.data('async-load') === 'true') {
                            asave.deferredValues[$thisElem.prop('id')] = value;
                            $thisElem.one('async-load', function (e) {
                                $(this).val(asave.deferredValues[$(this).prop('id')]);
                                delete asave.deferredValues[$(this).prop('id')];
    ...
    $(document).off('activesave-unload').on('activesave-unload', function (e, requestedTargets) {
        ...
        if (typeof requestedTargets !== 'undefined' && requestedTargets != null) {
            ...
            $requestedTargets.filter('form[id]').each(function () {
                if (asave.registeredForms[$(this).prop('id')]) {
                    targets[$(this).prop('id')] = asave.registeredForms[$(this).prop('id')];
                }
            });
        }
        else {
            targets = asave.registeredForms;
        }
        ...
        for (var name in targets) {
            $(document).one('activesave-persisted', function (e, scope, subscope) {
                ...
                asave.remove(null, subscope, scope);
            });
        }
    ...
    $(document).off('activesave-persist').on('activesave-persist', function (e, requestedTargets) {
        ...
        for (var name in targets) {
            console.log('Attempting to save ' + asave.getKey(targets[name].subscope, targets[name].scope));
    ...
    $(document).off('activesave-append').on('activesave-append', function (e, requestedTargets) {
        ...
        for (var targetName in targets) {
            ...
            for (var name in values) {
                asave.setField(target, target.form, name, values[name]);
            }
